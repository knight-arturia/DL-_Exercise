...
class TestGen(unittest.TestCase):
    ...
    def testMirroring(self):
        ...
        vertical = np.sum(original_corners[:, :, 1, :] -
                          vertical_augmented_corners,
                          axis=(1, 2))
        ...
        horizontal = np.sum(original_corners[:, 1, :, :] -
                            horizontal_augmented_corners,
                            axis=(1, 2))
        ...
        vertical_horizontal_augmented_corners = np.stack(
            list(
                zip(augmented_corners[:, 0, 0, :], augmented_corners[:, 1,
                                                                     1, :])))
        original_corner_diagonals = np.stack(
            list(
                zip(original_corners[:, 1, 1, :], original_corners[:, 0,
                                                                   0, :])))
        horizontal_vertical = np.sum(original_corner_diagonals -
                                     vertical_horizontal_augmented_corners,
                                     axis=(1, 2))
        ...
        np.testing.assert_almost_equal(
            np.sum(vertical * horizontal * horizontal_vertical), 0)
