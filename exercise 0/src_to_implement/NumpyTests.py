...
class TestCheckers(unittest.TestCase):
    ...
    def testReturnCopy(self):
        # Checks whether the output of the pattern is a copy of the output
        # object rather than the output object itself.
...
class TestCircle(unittest.TestCase):
    ...
    def testPattern(self):
        # Creates an image of a circle with resolution 1024x1024 a radius of
        # 200 with a center at
    ...
    def testPatternDifferentSize(self):
        # Creates an image of a circle with resolution 512x512 a radius of 20
        # with a center at
...
# @unittest.skipIf(NO_SPECTRUM, "Spectrum is optional") # making it optional -
# but might confuse one or another
#if SPECTRUM_TEST:
class TestSpectrum(unittest.TestCase):
    ...
    def testPattern(self):
        # Creates an RGB spectrum with resolution 255x255x3 and compares it to
        # the reference image
        import pattern
    ...
    def testPatternDifferentSize(self):
        # Creates an RGB spectrum with resolution 100x100x3 and compares it to
        # the reference image
        import pattern
...
class TestGen(unittest.TestCase):
    ...
    def testShuffle(self):
        ...
        # Since shuffle is enabled for one image generator the output should be
        # different.
        from generator import ImageGenerator
        gen = ImageGenerator(self.file_path,
                             self.label_path,
    ...
    def testRotation(self):
        ...
        batch1 = ImageGenerator(self.file_path,
                                self.label_path,
                                100, [32, 32, 3],
                                rotation=False,
                                mirroring=False,
                                shuffle=False).next()[0]
        batch2 = ImageGenerator(self.file_path,
                                self.label_path,
                                100, [32, 32, 3],
                                rotation=True,
                                mirroring=False,
        ...
        augmented_images_indices = np.sum(np.abs(batch1 - batch2),
                                          axis=(1, 2, 3)).astype(np.bool_)

        # extract corner points for each sample and reduce augmented to one
        # characateristic row
        ...
        # subtract characteristic corners to original corners.  after summing
        # spacial and channel dimensions, 0 are expected
        # e.g.  if sample 2 is rotated by 90, we have a 0 in rot1 at the 2nd
        # posistion see similiar to testMirroring
        rot1 = np.sum(original_corners[:, :, 0] - characteristic_corners,
                      axis=(1, 2))
        rot2 = np.sum(original_corners[:, 1, :] - characteristic_corners,
                      axis=(1, 2))
        rot3 = np.sum(original_corners[:, ::-1, 1] - characteristic_corners,
                      axis=(1, 2))

        # assumption is that augmented images are either rotated by 90 (rot1),
        # 180 (ro2) or 270 (rot3) degrees, thus
    ...
    def testMirroring(self):
        ...
        # pick vertical ref points and compare them with opposites in original
        # corners
        # compute then the diff and sum over vertical axis and channels.  those
        # images which were flipped vertically
        # contain now zeros
        vertical_augmented_corners = augmented_corners[:, :, 0]
        vertical = np.sum(original_corners[:, :, 1, :] - vertical_augmented_corners,
                          axis=(1, 2))

        # pick horizontal ref points and compare them with opposites in
        # original corners
        # compute then the diff and sum over horizontal axis and channels.
        # those images which were flipped horizontally
        # contain now zeros
        horizontal_augmented_corners = augmented_corners[:, 0, :]
        horizontal = np.sum(original_corners[:, 1, :, :] - horizontal_augmented_corners,
                            axis=(1, 2))

        # pick top left corner and bottom right corner (diagonals are flipped
        # if double mirror)
        vertical_horizontal_augmented_corners = np.stack(list(zip(augmented_corners[:, 0, 0, :], augmented_corners[:, 1,
                                                                     1, :])))
        original_corner_diagonals = np.stack(list(zip(original_corners[:, 1, 1, :], original_corners[:, 0,
                                                                   0, :])))
        horizontal_vertical = np.sum(original_corner_diagonals - vertical_horizontal_augmented_corners,
                                     axis=(1, 2))

        # the elementwise product of horizontal and vertical must be zero
        # since the images can only be augmented with vertical or horizontal
        # mirroring
        np.testing.assert_raises(AssertionError, np.testing.assert_array_equal,
                                 batch1, batch2)
    ...
    def testLabels(self):
        # this test makes sure your generator returns integers as labels and
        # not any strings or floating point values
        # if this one fails make sure you cast your array values to integers
        from generator import ImageGenerator
        label = ImageGenerator(self.file_path,
                               self.label_path,
                               12, [50, 50, 3],
                               rotation=False,
                               mirroring=False,
