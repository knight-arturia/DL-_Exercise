...
class RNN:
    ...
    def optimizer(self, Obj):
        self._optimizer = Obj
        self._optimizer1 = copy.deepcopy(Obj)
        self._optimizer2 = copy.deepcopy(Obj)
        self._optimizer3 = copy.deepcopy(Obj)
    ...
    '''
    access to W_hx and gradient_W_hx
    '''
    @property
    def gradient_weights(self):
        return self.gradient_W_hx
    @gradient_weights.setter
    def gradient_weights(self, Matrix):
        self.gradient_W_hx = Matrix
    @property
    def weights(self):
        return self.W_hx
    @weights.setter
    ...
    def backward(self, error_tensor):
        ...
        for i in np.arange(error_tensor.shape[0])[::-1]:
            ...
            e_h2 = np.dot(error_tensor[i] * d_sigmoid, self.W_y.T) + e_h
            ...
            if i == 0:
                d_W_hh = np.zeros_like(d_W_hh)
            else:
                d_W_hh = self.ht_tensor[i - 1].reshape((-1, 1))
            gradient_W_hx +=  np.vstack([d_W_hh, d_W_xh]) @ e_h1[:, None].T
            # print("gradient_W_xh shape: ", gradient_W_xh.shape)
            # print("gradient_W_hh shape: ", gradient_W_hh.shape)
            gradient_B_hx += e_h1
            W_xh = self.W_hx
            e_x = np.dot(e_h1, W_xh.T)[self.hidden_size:]
            e_h = np.dot(e_h1, W_xh.T)[:self.hidden_size]
        ...
        if self._optimizer:
            ...
            self.W_hx = self._optimizer2.calculate_update(self.W_hx, gradient_W_hx)
            self.B_hx = self._optimizer3.calculate_update(self.B_hx, gradient_B_hx)
